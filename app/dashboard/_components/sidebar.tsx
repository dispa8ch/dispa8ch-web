...
const Sidebar = () => {
  return (
    <aside className="w-[50px] box-content h-screen relative z-[5] bg-transparent column gap-8 py-3 items-center font-Inter_Medium px-3 border-r border-r-gray-300 transition-[width] duration-700 group hover:w-[170p] " >
      <section>
        <Link
          href={"/"}
      ...
      <section className='column gap-6 '>
        {sidebarLinks.map(({ link, icon: Icon, name }, i) => (
          ...
          >
            <Icon size={1.07} />
            <p className={`text-lg hidden group-hover:bloc  `}>{name}</p>
          </Link>
        ))}
      </section>
    </aside>
